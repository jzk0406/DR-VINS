...
void process()
{
    while (true)
    {
        ...
        {   
            printf("[SYNC] image_buf=%zu point_buf=%zu pose_buf=%zu\n",
            image_buf.size(), point_buf.size(), pose_buf.size());//newcode
            if (image_buf.front()->header.stamp.toSec() > pose_buf.front()->header.stamp.toSec())
            {   printf("[DROP] pose because image_front > pose_front | image=%.9f pose=%.9f\n",
                image_buf.front()->header.stamp.toSec(),
                pose_buf.front()->header.stamp.toSec());//new code
                pose_buf.pop();
                printf("throw pose at beginning\n");
            }
            else if (image_buf.front()->header.stamp.toSec() > point_buf.front()->header.stamp.toSec())
            {   printf("[DROP] point because image_front > point_front | image=%.9f point=%.9f\n",
                image_buf.front()->header.stamp.toSec(),
                point_buf.front()->header.stamp.toSec());//new code
                point_buf.pop();
        ...
        if (pose_msg != NULL)
        {
            ...
            printf(" image time %f \n", image_msg->header.stamp.toSec()); //changed
            // skip fisrt few
            ...
            if((T - last_t).norm() > SKIP_DIS)
            {
                ...
                printf("[KEYFRAME] before new KeyFrame, frame_index=%d, t=%.9f\n",
                       frame_index, pose_msg->header.stamp.toSec()); //newcode
                KeyFrame* keyframe = new KeyFrame(pose_msg->header.stamp.toSec(), frame_index, T, R, image,
                                   point_3d, point_2d_uv, point_2d_normal, point_id, sequence);   
                m_process.lock();
                start_flag = 1;
                ROS_WARN("[NODE] call posegraph.addKeyFrame frame_index=%d", frame_index);
...
int main(int argc, char **argv)
{
    ...
    if(argc != 2)
    {
        printf("please intput: rosrun loop_fusion hybrid_loop_fusion_node [config file] \n"
               "for example: rosrun loop_fusion hybrid_loop_fusion_node "
    ...
    std::string pkg_path = ros::package::getPath("hybrid_loop_fusion");
