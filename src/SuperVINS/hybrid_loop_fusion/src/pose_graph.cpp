...
{   ROS_WARN("[PG] enter addKeyFrame, global_index=%d flag_detect_loop=%d",
         global_index, flag_detect_loop);//new
    //shift to base frame
    Vector3d vio_P_cur;
    Matrix3d vio_R_cur;
    ...
    {
        TicToc tmp_t;
        ROS_WARN("[PG] before detectLoop, cur_index=%d", cur_kf->index);  
	...
	if (loop_index != -1)
	{
        ...
        if (old_kf != NULL)
        {
            ROS_WARN("[LOOP ACCEPT] cur=%d cand=%d (trusted by detectLoop)", cur_kf->index, loop_index);
...
{
    ROS_WARN("[DBOW] entered detectLoop frame=%d descriptor_num=%zu",
         frame_index, keyframe->brief_descriptors.size());//new
    ...
    printf("[DBOW] frame=%d ret_size=%zu\n", frame_index, ret.size());
    for (size_t i = 0; i < ret.size(); i++)
    {
        printf("[DBOW]   cand[%zu] id=%d score=%.6f\n",
           i, ret[i].Id, ret[i].Score);
    } //newcode
    ...
    if (ret.size() >= 1)
        printf("[DBOW] best_score=%.6f threshold=0.05\n", ret[0].Score);
    if (ret.size() >= 1 &&ret[0].Score > 0.05)
    {
        printf("[DBOW] enter candidate-check branch\n");    
        for (unsigned int i = 1; i < ret.size(); i++)
        {   printf("[DBOW]   compare cand[%u] id=%d score=%.6f\n",
                   i, ret[i].Id, ret[i].Score);//newcode
            //if (ret[i].Score > ret[0].Score * 0.3)
            if (ret[i].Score > 0.015)
            {   printf("[DBOW]   cand[%u] passes secondary threshold, id=%d score=%.6f\n",
                         i, ret[i].Id, ret[i].Score);//new
                find_loop = true;
                int tmp_index = ret[i].Id;
                if (DEBUG_IMAGE && 0)
                {
...
*/
    ...
    {
        // 对通过 DBoW 筛选的候选逐个做几何验证，谁先通过就返回谁
        for (unsigned int i = 1; i < ret.size(); i++)
        {
            if (ret[i].Score <= 0.015)
                continue;

            KeyFrame* old_kf = getKeyFrame(ret[i].Id);
            if (old_kf == NULL)
                continue;

            if (keyframe->findConnection(old_kf))
            {
                ROS_WARN("[DBOW] frame=%d return geo_index=%d", frame_index, ret[i].Id);
                return ret[i].Id;
            }
        }
    }

    ROS_WARN("[DBOW] frame=%d return -1", frame_index);
    return -1;
}
