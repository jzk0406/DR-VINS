...
{
    if (kpts0.empty() || kpts1.empty() || desc0 == nullptr || desc1 == nullptr)
    {
    ROS_WARN_STREAM_THROTTLE(1.0,
        "[EMPTY-GUARD] match_featurepoints: empty input, kpts0="
        << kpts0.size() << ", kpts1=" << kpts1.size()
        << ", desc0=" << (desc0 ? "ok" : "null")
        << ", desc1=" << (desc1 ? "ok" : "null"));
    return {};
    }
    ...
    std::vector<Ort::Value> input_tensors;
    size_t desc_dim = (extractor_type == SUPERPOINT) ? 256 : 128;
    ...
    input_tensors.push_back(Ort::Value::CreateTensor<float>(
        memory_info_handler, desc0, kpts0.size() * desc_dim ,//change number
