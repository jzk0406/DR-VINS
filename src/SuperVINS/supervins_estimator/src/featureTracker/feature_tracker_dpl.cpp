...
void FeatureTrackerDPL::match_features_dpl(cv::Mat prev_img_, cv::Mat cur_img_, vector<pair<cv::Point2f, vector<float>>> &prev_dplpts_descriptors_, vector<pair<cv::Point2f, vector<float>>> &cur_dplpts_descriptors_, vector<pair<int, int>> &result_matches,double &ransacReprojThreshold)
{
    ...
    if (tem_matches.empty())
    {
    ROS_WARN_STREAM_THROTTLE(1.0,
        "[EMPTY-GUARD] DPL matcher: tem_matches is empty, skip this round");
...
const size_t MIN_F_CHECK_PTS = 15;

if (points1.size() < MIN_F_CHECK_PTS ||
    points2.size() < MIN_F_CHECK_PTS ||
    points1.size() != points2.size())
{
    ROS_WARN_STREAM_THROTTLE(1.0,
        "[F-GUARD] skip F check in DPL matcher: pts1="
        << points1.size() << ", pts2=" << points2.size());
...
try
{
    cv::findFundamentalMat(points1, points2,
                           cv::FM_RANSAC, ransacReprojThreshold, 0.99, inliersMask);
}
catch (const cv::Exception &e)
{
    ROS_WARN_STREAM_THROTTLE(1.0,
        "[F-GUARD] findFundamentalMat exception in DPL matcher: " << e.what());
    result_matches = tem_matches;
    return;
}

if (inliersMask.size() != tem_matches.size())
{
    ROS_WARN_STREAM_THROTTLE(1.0,
        "[F-GUARD] invalid inliersMask size in DPL matcher: mask="
        << inliersMask.size() << ", matches=" << tem_matches.size());
    result_matches = tem_matches;
    return;
}
...
void FeatureTrackerDPL::match_with_predictions_dpl(cv::Mat prev_img_, cv::Mat cur_img_, vector<pair<cv::Point2f, vector<float>>> &prev_dplpts_descriptors_, vector<pair<cv::Point2f, vector<float>>> &cur_dplpts_descriptors_, vector<cv::Point2f> &predict_pts_, vector<cv::Point2f> &cur_pts_, vector<pair<int, int>> &result_matches,double &ransacReprojThreshold)
{
    ...
    {
    ROS_WARN_STREAM_THROTTLE(1.0,
        "[EMPTY-GUARD] match_with_predictions_dpl: matches is empty, skip this round");
    result_matches.clear();
    return;
    }
    // RANSAC 参数
    ...
    // 使用 RANSAC 进行模型估计
         std::vector<cv::Point2f> matched_prev_pts, matched_cur_pts;
matched_prev_pts.reserve(matches.size());
matched_cur_pts.reserve(matches.size());

for (const auto &m : matches)
{
    matched_prev_pts.push_back(prev_dplpts_normalized[m.first]);
    matched_cur_pts.push_back(cur_dplpts_normalized[m.second]);
}

const size_t MIN_F_CHECK_PTS = 15;
if (matched_prev_pts.size() < MIN_F_CHECK_PTS ||
    matched_cur_pts.size() < MIN_F_CHECK_PTS ||
    matched_prev_pts.size() != matched_cur_pts.size())
{
    ROS_WARN_STREAM_THROTTLE(1.0,
        "[F-GUARD] skip F check in match_with_predictions_dpl: prev="
        << matched_prev_pts.size() << ", cur=" << matched_cur_pts.size());
...
try
{
    cv::findFundamentalMat(matched_prev_pts, matched_cur_pts,
                           cv::FM_RANSAC, ransacReprojThreshold, 0.99, inliersMask);
}
catch (const cv::Exception &e)
{
    ROS_WARN_STREAM_THROTTLE(1.0,
        "[F-GUARD] findFundamentalMat exception in match_with_predictions_dpl: "
        << e.what());
    result_matches = matches;
    return;
}

if (inliersMask.size() != matches.size())
{
    ROS_WARN_STREAM_THROTTLE(1.0,
        "[F-GUARD] invalid inliersMask size in match_with_predictions_dpl: mask="
        << inliersMask.size() << ", matches=" << matches.size());
    result_matches = matches;
    return;
}
...
{
    const size_t MIN_F_CHECK_PTS = 15;
    if (cur_pts.size() < MIN_F_CHECK_PTS ||
        prev_pts.size() < MIN_F_CHECK_PTS ||
        cur_pts.size() != prev_pts.size())
    {
        ROS_WARN_STREAM_THROTTLE(1.0,
            "[F-GUARD] skip rejectWithF: cur_pts=" << cur_pts.size()
            << ", prev_pts=" << prev_pts.size());
        return;
    }
    ...
    {
        ROS_WARN_STREAM_THROTTLE(1.0,
            "[F-GUARD] findFundamentalMat exception in rejectWithF: " << e.what());
        return;
    }
    ...
    {
        ROS_WARN_STREAM_THROTTLE(1.0,
            "[F-GUARD] invalid status size in rejectWithF: status="
            << status.size() << ", cur_pts=" << cur_pts.size());
        return;
    }
